...
var RecentNotesPlugin = class extends import_obsidian3.Plugin {
  constructor() {
    ...
    this.savePath = async (file) => {
      ...
      const fileIndex = this.recentNotes.files.findIndex(({ path }) => file.path === path);
      if (fileIndex === -1) {
        this.recentNotes.files.unshift({
          path: file.path,
          filename: file.basename
        });
      } else {
        const [existingFile] = this.recentNotes.files.splice(fileIndex, 1);
        this.recentNotes.files.unshift(existingFile);
      }
